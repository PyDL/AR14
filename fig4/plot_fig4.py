...
for i, (ax, (title, x)) in enumerate(zip(axes, values.items())):
    ...
    handles = list(l1)
    labels = [f"{legend_plain} monthly mean (CC={cc_b:.2f})"]
    if i != 3:
        ...
        y_scatter = x[m]
        y_hi = max(float(np.nanpercentile(y_scatter, 99.5)), float(np.nanmax(upper)), float(np.nanmax(trend)))
        if i == 2:
            span = max(float(np.nanpercentile(np.abs(y_scatter), 99.5)), float(np.nanmax(np.abs(upper))), float(np.nanmax(np.abs(trend))))
...
for ax in axes:
    ...
    ax.tick_params(axis="x", rotation=30, labelsize=9.2)
